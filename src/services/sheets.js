...
NS.renderSyncPanel = function (el, opts) {
  ...
  const refreshExpiredToken = () => {
    ...
    if (!c.clientId || !window.google || !window.google.accounts || !window.google.accounts.oauth2) {
      return Promise.reject(new Error('لا يوجد Access Token صالح — أعد الربط بحساب Google أو ألصق توكن Access جديد (أو أضف Refresh Token للتجديد التلقائي)'));
    }
    if (window.location && window.location.protocol === 'file:') {
      return Promise.reject(new Error('لا يوجد Access Token صالح — ألصق توكن Access جديد (تسجيل الدخول عبر Google يحتاج تشغيل النظام عبر https)'));
    }
  ...
  wrap.querySelector('#gs-connect').addEventListener('click', () => {
