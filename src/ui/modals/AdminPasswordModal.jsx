  ...
  const submit = e => {
    ...
    if (!configured) {
      try {
        ...
        window.changeOwnPassword('', password)
    ...
    let ok = false
    try {
      ok = typeof window.verifyAdminPassword === 'function' ? window.verifyAdminPassword(password) : false
    } catch {
      ok = false
    }
    if (ok) {
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          <Button type="submit" variant="primary" icon={ShieldCheck}>
            {configured ? '✓ تأكيد' : '✓ تسجيل والمتابعة'}
          </Button>
        </div>
      </form>
    </Modal>
