...
function AddExpenseModalInner() {
  ...
  const handleSubmit = e => {
    ...
    if (!title.trim()) {
      showToast('يرجى إدخال بيان المصروف', 'error')
      return
    }
    const numAmount = Number(amount)
    if (isNaN(numAmount) || numAmount <= 0) {
      showToast('يرجى إدخال قيمة مصروف صحيحة أكبر من الصفر', 'error')
      return
    }
    if (recurring && (!dueDay || Number(dueDay) < 1 || Number(dueDay) > 31)) {
      showToast('يرجى إدخال يوم استحقاق شهري صحيح بين 1 و 31', 'error')
      return
    }
