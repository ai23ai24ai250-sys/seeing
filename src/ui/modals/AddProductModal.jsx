...
function AddProductModalInner() {
  ...
  const supplierOptions = useMemo(
    () => [{ value: '', label: '-- اختر المورد المصنع --' }, ...suppliers.map(s => ({ value: s.id, label: s.phone ? `${s.name} (${s.phone})` : s.name }))],
    [suppliers]
  )
    ...
    >
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Input label="المخزون الحالي" type="number" value={stock} onChange={setStock} required className="text-center" />
          <Input label="سعر الشراء" type="number" value={purchasePrice} onChange={setPurchasePrice} required className="text-center" />
          <Input label="سعر البيع" type="number" value={sellingPrice} onChange={setSellingPrice} required className="text-center" hint="سعر بيع القطعة للعميل" />
        </div>

        <Input label="الحد الأدنى للمخزون للتنبيه" type="number" value={minStock} onChange={setMinStock} className="text-center text-amber-300" hint="عند انخفاض المخزون لهذه النسبة يظهر في النواقص" />
