...
const CHAT_HISTORY_KEY = 'bms_ai_chat_history'
...
function AiAssistantModalInner() {
  ...
  const send = text => {
    ...
    setTyping(true)
    askAiProvider(config, question, buildContextForPrompt(data), { tools: AI_TOOLS, history, scope }).then(reply => {
      setTyping(false)
      if (reply) {
  ...
  const sendButtonDisabled = !input.trim()
