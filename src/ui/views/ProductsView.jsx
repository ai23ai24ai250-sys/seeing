...
function ProductRow({ product, onRefresh }) {
  ...
  const remove = () => {
    ...
    window.deleteProduct(product.id).then(ok => {
      if (ok) showToast(`تم حذف المنتج "${product.name}" بنجاح`, 'info')
      onRefresh()
    })
  }
    ...
    <tr className={status === 'negative' ? 'bg-rose-950/20' : status === 'low' ? 'low-stock-row' : ''}>
      ...
      <td>
        <div className="flex items-center gap-2 whitespace-nowrap">
          ...
          <Button size="sm" variant="ghost" icon={Trash2} onClick={remove} className="text-rose-300 hover:bg-rose-500/10">
            حذف
          </Button>
        </div>
      </td>
          ...
          <table className="data-table w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map(p => <ProductRow key={p.id} product={p} onRefresh={refresh} />)
              )}
            </tbody>
          </table>
        </div>
