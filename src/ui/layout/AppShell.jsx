...
// 🔒 RBAC للنقل — نفس قواعد updateUserUI في legacy: أمين المخزن يرى المنتجات فقط،
// وموظف المبيعات يخفي (المستخدمون/التقارير/الموردون/المدفوعات)، والمدير يرى الكل.
// الزائر (null) يُستخدم في الاختبارات فقط ويشاهد الكل.
function visibleNav(role) {
  if (!role) return NAV
  if (role === 'storekeeper') return NAV.filter(item => item.id === 'products')
  if (role === 'employee') {
    const hidden = ['users', 'reports', 'suppliers', 'payments']
    return NAV.filter(item => !hidden.includes(item.id))
  }
  return NAV
}
...
function AppShell() {
  ...
  const [active, setActive] = useState(() => (role === 'storekeeper' ? 'products' : 'dashboard'))
    ...
    <div dir="rtl" className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          <header className="h-16 flex items-center justify-between px-4 sm:px-6 border-b border-slate-800 bg-slate-900/40 sticky top-0 z-10 gap-2">
            <button
              type="button"
              onClick={() => setActive('dashboard')}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="md:hidden w-9 h-9 grid place-items-center rounded-xl border border-slate-700 text-slate-300 hover:bg-slate-800 shrink-0 cursor-pointer"
              >
              ...
              <button
                ...
                className="h-9 px-3 flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-700 to-emerald-600 hover:from-emerald-600 hover:to-emerald-500 text-white text-sm font-bold shadow-lg shadow-emerald-700/30 whitespace-nowrap transition-all cursor-pointer shrink-0"
              >
              ...
              <button
                ...
                className="h-9 px-3 flex items-center gap-2 rounded-xl bg-amber-600 border border-amber-500 hover:bg-amber-500 text-white text-sm font-bold whitespace-nowrap transition-all cursor-pointer shrink-0"
              >
              ...
              <button
                ...
                className="h-9 px-3 flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/80 text-slate-200 hover:bg-slate-800 text-sm font-bold whitespace-nowrap transition-all cursor-pointer shrink-0"
              >
              ...
              <div className="relative shrink-0">
                <button
                  ...
                  className="h-9 px-3 flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/80 text-slate-300 hover:bg-slate-800 text-sm font-medium transition-all whitespace-nowrap cursor-pointer"
                >
